---
title: "Finals - Computational Mathematics"
author: "Monu Chacko"
date: "5/21/2020"
output: 
  html_document:
...
```{r}
# Data Load
kdata = read.csv("train_house.csv", header = TRUE) %>% filter(GrLivArea < 4000)

#kdata = read.csv(text = getURL("https://raw.githubusercontent.com/IsARam/DATA605/master/train%5B1%5D.csv?_sm_au_=iVV6f56RMnTkt5jMkRvMGK3JRp2ft"), header = TRUE) %>% # Outlier Removal filter(GrLivArea < 4000)
...
```{r}
kdatatest = read.csv("test_house.csv", header = TRUE) %>% filter(GrLivArea < 4000)

#kdatatest = read.csv(text = getURL("https://raw.githubusercontent.com/IsARam/DATA605/master/test%5B1%5D.csv?_sm_au_=iVV1fQtQ75TPVL5NkRvMGK3JRp2ft"), header = TRUE) %>% # Outlier Removal filter(GrLivArea < 4000)
