...
####genhlth is of type 'categorical' with the levels mentioned above.
...
####exerany is a 'nominal categorical'. There is no measure associated to the number. They are used to differenciate between objects.

```{r}
typeof(cdc$hlthplan)
```

####hlthplan is a 'nominal categorical'. There is no measure associated to the number. They are used to differenciate between objects.

```{r}
typeof(cdc$smoke100)
```

####smoke100 is a 'nominal categorical'. There is no measure associated to the number. They are used to differenciate between objects.

```{r}
typeof(cdc$height)
```

####height is a 'numerical continuous' (quantitative).

```{r}
typeof(cdc$weight)
```

####weight is a 'numerical continuous' (quantitative).

```{r}
typeof(cdc$wtdesire)
```

####wtdesire is a 'numerical continuous' (quantitative).

```{r}
typeof(cdc$age)
```

####age is a 'numerical continuous' (quantitative).
...
####gender is of type 'categorical' with the levels mentioned above.
...
```{r}
males_col <- nrow(cdc[cdc$gender == "m", ])

#females_col <- nrow(cdc[cdc$gender == "f", ])
#males_col + females_col

cat("Number of Males: ", males_col)

genhlth_gooehealth_col <- nrow(cdc[cdc$genhlth == "good", ])
#we can also get the data from summary for quick manual calculation 
#summary(cdc$genhlth)
genhlth_gooehealth_col
cat("Good Health Proportion: ", (genhlth_gooehealth_col/20000))
...
###Answer 3:
####The mosiac plot shows us that more males smoked atleast less than 100 cigarettes in their lifetime than females. This plot makes comparision of two variables easier.  
...
summary(under23_and_smoke$age)
summary(under23_and_smoke$smoke100)
```
...
###Answer 5:
####The box plot shows us that there is a correlation between BMI and health. The BMI for poor health has a larger range (spread) than excellent health.  

####Box plot - BMI vs Gender
####Gender is a binary column (male or female) and makes it difficult for find correlation like the general health column. It is interesting because in the earlier analysis we found that females smoked lesser than their male counterparts. This could through some light into the relationship between smoking and BMI for further investigations. From the box plot below it doesn't appear to have any relation. The female polulation has a larger spread of BMI than males. 

```{r boxplot-age, eval=TRUE}
...
plot(cdc$weight, cdc$wtdesire, main="Weight vs Desired Weight", 
    xlab="Weight", ylab="Desired Weight", pch=20) 

```

####Weight is the actual and desired wieght is the target. They have a linear best fit line. Most of the data is under the line suggesting that the target is not achieved yet.
...
###Answer:
####wdiff contains kpi information indicating the performance of a person. In other words it shows if a person achieved their target. If the value is 0 then the person has achieved their target. If the value is negative or positive then the person did not achieve the target. It does not mean underperformance because a person could have a target of weight (bmi) loss of weight (bmi) gain. However the farther away from 0 would indicate then they need to do more.   
...
quantile(cdc$wdiff)
summary(cdc$wdiff)
hist(cdc$wdiff, freq = FALSE)
lines(density(cdc$wdiff), col="red")
```


```{r}
plot(density(cdc$wdiff), col="red")
density(cdc$wdiff)
```

####This is a unimodal normal distribution. People seem to be working towards their target. The graph has a slight left skew. 
...
density(male_wdiff$wdiff)
density(female_wdiff$wdiff)
summary(male_wdiff$wdiff)
summary(female_wdiff$wdiff)
boxplot(cdc$wdiff ~ cdc$gender)
```

####The stats and box plot suggest that they different. The range for male is larger than female.
...
quantile(cdc$weight)
summary(cdc$weight)
hist(cdc$weight, freq = FALSE)
lines(density(cdc$weight), col="red")

```

####Weight is right skewed or positively skewed.

```{r}
mwg <- mean(cdc$weight)
swg <- sd(cdc$weight)
stdev <- subset(cdc, cdc$weight < (mwg + swg) & cdc$weight > (mwg - swg))
dim(stdev)[1]/dim(cdc)[1]
```
